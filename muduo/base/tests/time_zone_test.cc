...
void test(const TimeZone &tz, TestCase tc)
{
  ...
  {
    struct tm local = tz.to_local_time(gmt);
    char buf[256];
...

  {
    ...
    time_t result = tz.from_local_time(local);
    if (result != gmt)
    {
      struct tm local2 = tz.to_local_time(result);
      char buf[256];
      strftime(buf, sizeof buf, "%F %T%z(%Z)", &local2);

      printf("WRONG from_local_time: %ld %ld %s\n",
...
void testUtc()
{
  ...
  for (time_t t = -kRange; t <= kRange; t += 11)
  {
    ...
    struct tm t2 = TimeZone::to_utc_time(t, true);
    ...
    time_t t3 = TimeZone::from_utc_time(t2);
    if (t != t3)
    {
